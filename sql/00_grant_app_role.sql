...
-- which reads `massive_app` as a Databricks principal (user/group/service
...
--
--   1. A notebook PYTHON cell, connecting to Postgres directly:
--
...
--        conn.cursor().execute("GRANT CREATE ON SCHEMA public TO massive_app")
--
--      Note: this connects AS massive_app, which cannot grant to itself. Run it
--      as a superuser role, or use option 2/3 below.
--
--   2. The Lakebase instance's own query editor in the Databricks UI (opened
--      from the database instance page - not the generic SQL editor, which
--      targets Unity Catalog).
--
--   3. psql, authenticated as your Databricks identity:
...
-- not CREATE, so app.py's ensure_table() / ensure_watchlist_table() /
